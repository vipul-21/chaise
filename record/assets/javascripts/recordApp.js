...
var chaiseRecordApp = angular.module("chaiseRecordApp", ['kendo.directives','ngResource', 'ngRoute', 'ngAnimate', 'ui.bootstrap', 'ngCookies', 'ngSanitize']);
...
// Kendo Grid UI controller (experimental)
chaiseRecordApp.controller('KendoDetailCtrl', ['$rootScope', '$scope','ermrestService', 'schemaService', 'configService', 'locationService', 'notFoundService',
    function($rootScope, $scope, ermrestService, schemaService, configService, locationService, notFoundService)
    {
        $scope.save = function (entity) {
            var savedState = kendo.stringify($scope.kendoDetailGrid.getOptions());
            localStorage["kendo-grid-options:" + entity.tableName] = savedState;
        };

        $scope.load = function (entity) {
            var savedState = localStorage["kendo-grid-options:" + entity.tableName];
            if (savedState) {
                $scope.kendoDetailGrid.setOptions(JSON.parse(savedState));
            }
        };

        $scope.gridOptions = function(entity)
        {
            if (!entity.open) {
                return;
            }
            ...
            var columnMetadata = [];
            var displayColumns = schemaService.getDisplayColumns(entity.tableName);
            for (var column in displayColumns) {
                if (displayColumns.hasOwnProperty(column)) {
                    var displayName = displayColumns[column];
                    columnMetadata.push({"field": column, "title": displayName, "width": 200})
                }
            }
...
/*                    schema: {
                ...
                /*columnResize: $scope.save(entity),
                columnShow: $scope.save(entity),
                columnHide: $scope.save(entity),
                columnReorder: $scope.save(entity),
                columnLock: $scope.save(entity),
                columnUnlock: $scope.save(entity),*/
                dataBound: function(e)
                {
                    ...
                    //var grid = e.sender;
                    //kendo.ui.progress($("#kendoDetail"), true);
                    //for (var i = 0; i < grid.columns.length; i++) {
                    //    grid.autoFitColumn(i);
                    //}
                    //kendo.ui.progress($("#kendoDetailGrid"), false);
                    /*for (var i = 0; i < grid._data.length; i++) {
                        var row = grid._data[i];
                        for (var column in row) {
                            if (row.hasOwnProperty(column)) {
                                var value = row[column];
                                if ( value != null && typeof value === "string" && value.length > 10) {
